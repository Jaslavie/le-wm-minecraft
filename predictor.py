...
class Transformer(nn.Module):
    ...
    """
    def __init__(self, attention_heads, embedding_dim, action_dim, mlp_hidden_nodes, dropout, history_len):
        super().__init__()
        ...
        self.adaln2 = AdaLn(action_dim, embedding_dim)
    ...
    def forward(self, obs, action):
        ...
        obs = self.attention_heads(obs, obs, obs, attn_mask=self.causal_mask)[0]
        obs = obs + residual1
...
class AdaLn(nn.Module):
    ...
    dynamically at inference time. Normally, this is fixed as a model weight.
        input: condition vector of prior actions
    ...
    def __init__(self, action_dim, embedding_dim):
        ...
        self.cond_proj = nn.Sequential(
            nn.Linear(action_dim, embedding_dim * 2), # 192 dim * 2 
            nn.SELU(),
        )
...
class Predictor(nn.Module):
    ...
    def forward(self, obs, actions):
        ...
        # apply projection head to predict the next observation embedding
        obs_t_next = self.projection_head(obs[:, -1])
