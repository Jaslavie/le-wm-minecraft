"""
This is created by christ to speed up the planning
(delete this file and redirct in __init__.py to revert the fix)
...
class Planner:
    ...
    def objective_function(self, z_curr, z_g):
        ...
        """
        if z_g.dim() == 1:
            z_g = z_g.unsqueeze(0).expand_as(z_curr)

        return F.mse_loss(z_curr, z_g, reduction='none').mean(dim=1)
    ...
    def planner(self, lewm, obs, obs_goal, cam_mean, cam_std, sigreg_lambd=0.1, warm_start=None):
        ...
        for cem_iter in range(self.max_iter):
            ...
            samples_tensor = torch.as_tensor(samples, dtype=torch.float32, device=device)
            ...
            for t in range(self.horizon):
                ...
                next_emb = lewm.predictor(obs_context, act_context)
                z_pred = next_emb[:, -1:, :]
                # z_pred_hist = [[z[i] for z in z_pred] for i in range(batch_size)]
                z_pred_hist.append(z_pred)

            # final predicted embedding at end of horizon (192,)
            z_H = z_pred_hist[-1].view(batch_size, -1)

            # 3. Compute cost: how close imagined final state is to fixed goal zg
            # score = self.objective_function(z_H, zg).item()
            scores = self.objective_function(z_H, zg.view(1, -1).squeeze(0))
            rollout_hists = [[z[i] for z in z_pred_hist] for i in range(batch_size)] # Previously z_pred_hist
            ...
            elite_idx = torch.topk(-scores, self.n_elites).indices
            elites = samples_tensor[elite_idx]
            top5 = [scores[i].item() for i in elite_idx[:5]]
            print(f"top 5 best scores: {', '.join(f'{s:.4f}' for s in top5)}")
            mu = elites[..., 8:].mean(dim=0).cpu().numpy()
            sigma = elites[..., 8:].std(dim=0).cpu().numpy() + 1e-6
            # update action probabilities based on elites
            p = elites[..., :8].mean(dim=0).cpu().numpy()
            p = np.clip(p, 0.05, 0.95)
            
            # get best elite score for metrics
            iter_best_elite_idx = elite_idx[0]
            iter_best_score = scores[iter_best_elite_idx]
            iter_best_actions = samples_tensor[iter_best_elite_idx]
            iter_best_z_pred_hist = rollout_hists[iter_best_elite_idx]

            if iter_best_score < best_score:
                best_score = iter_best_score
                best_actions = iter_best_actions.clone()
                best_z_pred_hist = [z.clone() for z in iter_best_z_pred_hist]

            print(f"Planner iteration: {cem_iter + 1}/{self.max_iter}: {best_score:.4f}")

        # 4. Action selection: return the mean of elites
        z_H = best_z_pred_hist[-1].view(1, -1)
        ...
        return best_actions, planning_losses, distribution_params
